...
def sanitize_filename(filename, video_date, video_id):
    # Convert video date string to datetime object and format it
...
def download_video(video_url, video_title, video_id, video_date, format='mp3', output_dir='/mnt/youtube'):
    global cancel_download
    
    # Create formatted filename
    formatted_filename = sanitize_filename(video_title, video_date, video_id)
    output_path = os.path.join(output_dir, f'{formatted_filename}.{format}')
    ...
    ydl_opts = {
        ...
        'outtmpl': os.path.join(output_dir, f'{formatted_filename}.%(ext)s'),
        'postprocessors': [{
...
def start_download(videos, format, output_dir):
    ...
    for i, video in enumerate(stqdm(videos)):
        ...
        download_video(video_url, video_title, video_id, video_date, format, output_dir)
